...
function cg_results(df::DataFrame, f::FormulaTerm, n_iters::Int; burn_in::Int=1, seed=16, accuracy_cg=1e-8)
    ...
    function _cg!(x, Q, b, iters; Pl="jac")
        ...
        elseif Pl == "ichol"
            F = lldl(Q)
            # if sum(F.D .<= 0.0) > 0
            #     println("$(sum(F.D .<= 0.0)) negative entries")
            #     F.D .= abs.(F.D)
            # end
...
if false
    for N in [70000]
        ...
        FileIO.save(path_to_folder*"results_GG_lldl.jld2", "results_sim", results_sim, "results_real", results_real, "outputs_sim", outputs_sim, "outputs_real", outputs_real)
    end
end


results_sim, outputs_sim, results_real, outputs_real = load(path_to_folder*"results_GG_lldl.jld2", "results_sim", "outputs_sim", "results_real", "outputs_real");
...
CSV.write(path_to_folder*"summary_GG_lldl.csv", pretty_df)